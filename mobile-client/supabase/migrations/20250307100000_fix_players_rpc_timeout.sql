-- Fix statement timeout: remove correlated subqueries, use single-pass aggregations
...
AS $$
  ...
  -- Single-pass totals from full boxscores (no correlated subqueries)
  player_totals AS (
    SELECT
      athlete_id::text,
  ...
  -- Limited game_log (last 30 games) in separate pass
  game_log_limited AS (
    SELECT
      athlete_id::text,
      COALESCE(

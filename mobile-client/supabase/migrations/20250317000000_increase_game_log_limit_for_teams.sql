-- Increase game_log limit from 20 to 60 in get_players_enhanced_for_teams
-- So "vs OPP: X PPG in N games" in Key Matchups includes head-to-head history
-- (20 games was too short - DET vs MIA games fell outside the window)

CREATE OR REPLACE FUNCTION get_players_enhanced_for_teams(
  p_season int DEFAULT 2026,
  p_season_type int DEFAULT 2,
  p_team_abbrevs text[] DEFAULT '{}'
)
RETURNS TABLE (
  athlete_id text,
...
AS $$
  ...
  boxscores AS (
    ...
    WHERE b.season = p_season
      ...
      AND (cardinality(p_team_abbrevs) = 0 OR b.team_abbreviation = ANY(p_team_abbrevs))
  ),
  player_totals AS (
    SELECT
      athlete_id::text,

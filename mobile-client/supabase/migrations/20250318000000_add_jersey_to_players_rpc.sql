-- Add jersey_number to get_players_enhanced (from player_boxscores_raw.athlete_jersey)
...
AS $$
  ...
  player_totals AS (
    SELECT
      ...
      NULLIF(TRIM(COALESCE((array_agg(athlete_jersey ORDER BY game_date DESC))[1]::text, '')), '') AS jersey_number,

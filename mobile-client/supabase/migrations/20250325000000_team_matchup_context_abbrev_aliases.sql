-- Match team_abbreviations variants (e.g. NY vs NYK) when resolving active player IDs
-- so active_*_ids stay aligned with schedule abbreviations and client roster logic.
...
BEGIN
  ...
  -- Active away player IDs (played in last N games) — include all DB variants for team (NY/NYK, etc.)
  WITH away_canonical AS (
    SELECT COALESCE(
      (SELECT canonical FROM team_abbreviations WHERE UPPER(TRIM(variant)) = v_away LIMIT 1),
      v_away
    ) AS c
  ),
  away_variants AS (
    SELECT UPPER(TRIM(variant)) AS abbrev
    FROM team_abbreviations
    WHERE canonical = (SELECT c FROM away_canonical)
    UNION
    SELECT v_away
  ),
  away_games_distinct AS (
    SELECT DISTINCT pb.game_id, pb.game_date
    FROM player_boxscores_raw pb
    CROSS JOIN away_variants av
    WHERE pb.season = p_season
      AND pb.season_type = 2
      AND UPPER(TRIM(pb.team_abbreviation)) = av.abbrev
      AND (pb.did_not_play IS NULL OR pb.did_not_play = false)
      AND pb.game_id::text NOT IN ('401809839','401838140','401838141','401838142','401838143')
  ),
  away_last_games AS (
    SELECT game_id FROM (
  ...
  SELECT COALESCE(array_agg(DISTINCT b.athlete_id::text), ARRAY[]::text[])
  INTO v_active_away
  ...
  WHERE b.season = p_season
    AND b.season_type = 2
    AND UPPER(TRIM(b.team_abbreviation)) = av.abbrev
    AND (b.did_not_play IS NULL OR b.did_not_play = false);

  -- Active home player IDs
  WITH home_canonical AS (
    SELECT COALESCE(
      (SELECT canonical FROM team_abbreviations WHERE UPPER(TRIM(variant)) = v_home LIMIT 1),
      v_home
    ) AS c
  ),
  home_variants AS (
    SELECT UPPER(TRIM(variant)) AS abbrev
    FROM team_abbreviations
    WHERE canonical = (SELECT c FROM home_canonical)
    UNION
    SELECT v_home
  ),
  home_games_distinct AS (
    SELECT DISTINCT pb.game_id, pb.game_date
    FROM player_boxscores_raw pb
    CROSS JOIN home_variants hv
    WHERE pb.season = p_season
      AND pb.season_type = 2
      AND UPPER(TRIM(pb.team_abbreviation)) = hv.abbrev
      AND (pb.did_not_play IS NULL OR pb.did_not_play = false)
      AND pb.game_id::text NOT IN ('401809839','401838140','401838141','401838142','401838143')
  ),
  home_last_games AS (
  ...
  WHERE b.season = p_season
    AND b.season_type = 2
    AND UPPER(TRIM(b.team_abbreviation)) = hv.abbrev
    AND (b.did_not_play IS NULL OR b.did_not_play = false);

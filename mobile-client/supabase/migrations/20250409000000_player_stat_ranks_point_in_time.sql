-- Add optional p_as_of_date to get_player_stat_ranks for point-in-time league rank
-- When provided, only boxscores before that date are counted (PIT snapshot).
-- When NULL, full season stats are used (existing behavior for mismatch alerts).

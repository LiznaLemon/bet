-- Add team_gp * 0.7 qualification threshold to get_player_stat_ranks
-- to align rankings with get_players_paginated (which already applies this filter).
-- Also maintains the p_as_of_date PIT support added in 20250409000000.
-- Must DROP first because we're replacing the function.
...
AS $$
  ...
  -- Team game counts (PIT-aware): how many games each team played in the window
  team_game_counts AS (
  ...
  -- Only rank qualified players (same threshold as get_players_paginated)
  qualified AS (
    SELECT pt.*
